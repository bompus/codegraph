...
| TypeScript/JS | Angular / NgRx | `store.dispatch(action)` → `ofType(action)` effect body | S | ✅ **ngrx-dispatch synthesizer** (`ngrxEffectEdges`, `synthesizedBy:'ngrx-dispatch'`): 2-pass — Pass 1 maps each action key (last dot-segment: `UsersActions.loadUsers`→`loadUsers`, or legacy class name `LoadThings`) → every effect node subscribing via `ofType(...)`, covering class-field `x$ = createEffect(…)` (method), functional `export const x$ = createEffect(…)` (constant, signature-gated), and legacy `@Effect() x$ = …pipe(ofType…)` (property) shapes incl. multi-arg/multi-`ofType`; Pass 2 links each `*.store.dispatch(…)` site's enclosing fn → the subscribing effects, resolving the arg as `new X()`, creator call `Ns.x()`, or a backward-scanned local/param (`const a = xCreator()`, `a: X`). Receiver gated `/store/i` + action must be registered → **7 edges on ngrx/platform example-app (all genuine), 0 on the NgRx-free angular-realworld control**. `{dispatch:false}` effects register; effect→effect dispatch free. ✅ +`ngrx-select` read edges (`*.store.select(sel)`/`selectSignal(sel)` → selector node — effects' `concatLatestFrom`/`withLatestFrom` args AND component/guard reads, file-gated on `@ngrx` import + `/store/i` receiver; selector-gated, import-pinned, ambiguity declines — **13 edges on example-app, all genuine**); ✅ destructured selector factories shipped (#88) — exported `const { x } = f()` bindings now mint constant nodes + `getSelectors`/`getRouterSelectors` joined the selector-sig gate: example-app **13→15 `ngrx-select` edges**, `updateTitle$ → selectRouteData` connected; 🔬 NgRx Signal Store (`signalStore`/`rxMethod`), string-typed `ofType('[…]')` args |
...
- **Spring/MyBatis enterprise flow (validated 2026-05-26, mall-tiny S — closes #389).** Three holes that left
  ...
  Coverage frontier: UNQUALIFIED cross-module XML statement references (`<include refid="cols">` pointing at
  a fragment in another mapper file — a same-namespace lookup can't cross files); `@PropertySource` external
  property files (assessed — config binding, not dispatch); Spring Cloud Config (remote properties); ambiguous
  mapper-name collisions across packages (Java mapper `com.a.X` and `com.b.X` both with `selectOne` — currently
  dropped to avoid mis-resolving). ✅ cross-mapper `<include refid="com.example.MapperB.frag">` shipped
  (2026-07): a dotted refid is qualified on the LAST dot only, matching the fragment's `<ns>::<id>`
  qualifiedName (a blanket `.`→`::` mangled multi-dot namespaces into `com::example::…`).
