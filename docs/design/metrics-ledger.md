...
Same-build check (2026-09-16, both arms on the post-merge binary the env arms used, back-to-back): store 67.3s no-env vs 65.5s `PD=1000` — **the halving was the merge's build change, not the env var**; robobun's confound call confirmed. Wall 356.0 vs 316.6s sits inside the observed run spread (identical env arms were 327.7 vs 295.3s); faults ~19–20M and sys time ~185–207s show no purge-delay signature either way. Corrective follow-up posted on #42942. Note the store improvement itself is real — the upstream merge + reconcile halved the bulk-insert leg (132.5s pre-merge → ~66s post-merge, env-independent); mechanism unattributed.
