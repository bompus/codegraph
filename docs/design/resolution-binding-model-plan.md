...
7. Retire the spine: `resolveOneInner` becomes kernel call → framework merge; the `name-matcher.ts` strategies and the parity suite's TS half go; the dump gate and golden dumps carry correctness. Scoped 2026-09-26 (census of every gate corpus plus a caller survey) into sub-legs, each landing on its own:
   ...
   - **7b. The deferred passes.** The conformance chain pass (`resolveChainedCallsViaConformance`: matchMethodCall, matchScopedCallChain, matchDottedCallChain) and the `this.<member>` pass call name-matcher directly. The kernel's `defer` punts feed them: exposed 1,752, Ocelot 1,373, ktor 1,213, javalin 884.
   ...
   - **7d. Residual punts on migrated languages:** `member-tail` (store-accessor `().` receivers and others; vitest 287), `store-bind` and resolveStoreAction (exposed 282 JS), supertype walks in pool batches before the snapshot refresh (`rmot-`/`btm-supers`, laravel 109, vitest 101), `mc-tfield-ambig`, `claimed`, `gated-import`, `chain`.
