...
5. **Adjacent enhancement candidates** (different axis — node-set changes): C `ops->read` fn-ptr field calls (~12k refs, §5.22 — needs field nodes + C receiver inference); `unknown-receiver` genuine misses (local-var data-flow — a different problem entirely); `Self::AssocType::*` binding — **done 2026-09-19** (§5.28).
