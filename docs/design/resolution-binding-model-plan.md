...
7. Retire the spine: `resolveOneInner` becomes kernel call → framework merge; the `name-matcher.ts` strategies and the parity suite's TS half go; the dump gate and golden dumps carry correctness. Scoped 2026-09-26 (census of every gate corpus plus a caller survey) into sub-legs, each landing on its own:
   ...
   - **7f. Delete.** Needs 7b–7e, a kernel-backed `resolveViaImport` for the one framework resolver that calls it directly (expo-modules), the shared import helpers moved out of the spine, and a decision on `CODEGRAPH_KERNEL_RESOLVE=0`: without a TypeScript spine the escape hatch and the dump gate's reference side both go.
