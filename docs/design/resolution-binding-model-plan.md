# Resolution binding model as the one source of truth for exports and bindings
...
- **The main-thread kernel conn is closed the moment the pool engages**, because its `readPendingBatch`/`resolveChunk` can't run while worker node conns are on the live `-shm` (cross-build again). Batch reads fall back to `getUnresolvedReferencesBatchAfter`; the conn re-initializes lazily for post-pool passes.
- **The WAL valve is quiesced while a kernel conn is live on the real `-shm`**, because its off-thread checkpoint workers are the remaining cross-build shm writers. `journal_size_limit` still bounds the WAL during that window (inline, same-thread). The valve resumes on kernel close, and deferred post-passes get the same quiesce in `index.ts`.
- **Kernel conns must close deterministically, never at GC time** (`KernelResolver.close()`, with `conn: Option<Connection>` and `take()` on call). The first snapshot-only cut still SIGBUS'd once in five linux runs: a dropped-but-unclosed main-thread conn's destructor fired mid-run on GC timing, and its shm teardown raced worker conns. Ordering rule: `close()` runs only while the valve is stopped and no worker has attached (before the snapshot fold, before `tryCreate`, or after `pool.destroy()`); index/sync `finally` blocks call `resolver.closeKernel()` so no conn outlives a run into the next one's armed valve. The batch-loop `backpressure` hook and the index-recreate fold are also gated on no-live-kernel, because `WalCheckpointValve.stop()` clears only the timer; explicit `backpressure()`/`foldNow()` calls still do off-thread shm work.
...
1. **Attribution first**, with `CODEGRAPH_RESOLVE_PROFILE=2` per-ref punt reasons, so every intentional TS fallback is measurable.
...
5. **Scoped `function_ref`** (`match_function_ref_scoped` for `Cls::member` member-pointer refs) plus the non-bare `resolveViaImport` member-descent step of the function_ref block (Leg D).
...
**Deliberately not ported**: `resolveThisMemberFnRef` (`this.` function_refs, a class-scope walk), `matchByExactName`, supertype walks, storeAccessorChain. Each is reachable only through an attributed punt; the kernel never fabricates an edge the TS path wouldn't produce.

### Phase 5b: Rust resolution (legs R1–R5 landed 2026-09-18)

Rust is not a `BINDINGS_LANGUAGES` member, since its walker (`rustlang.rs`) emits nodes/refs but no binding rows, but the audit showed ~29k Rust refs resolve TS-side through generic arms **without bindings** (empty on both sides), so bindings emission turned out *not* to be the gate. Five legs landed:
...
- **R3+R4.** `self.` receiver arms + inheritance locality (`§5.25`): `match_rust_self_call` (owner = enclosing impl off the caller's qname) and `match_rust_self_field_call` (field type regexed off the owner struct's decl lines, normalized through a ported `rustFieldTypeName` that strips `&`/`mut`/`Box`/`Rc`/`Arc`/`dyn`/`impl`/generics, declines containers and returns exclusive-null on inference failure) wired at the exact TS slot; `is_bound_to_out_of_repo_import` gained the rust branch (ported `collectRustUseBindings` + stdlib roots + `resolveRustModuleFile`), taking the `rust-inh` bucket to **0**. `member-tail` −1,934, handled +18,040 → **99.6%**. **Divergence caught**: removing the dot-gate entirely let strat arms resolve `x.y` receivers at 0.7/0.8 where TS's unported `inferLocalReceiverType` gives 0.9, the same target with fabricated metadata; the gate now punts dotted rust refs unless they're `self.`-rooted calls.
...
5. **Adjacent enhancement candidates** (a different axis, node-set changes): ~~C `ops->read` fn-ptr field calls~~ **LANDED as enhancement 2026-09-19** (§5.33): callable fn-pointer members extract as `field` nodes (`Type::member`), a C receiver-decl arm feeds `inferLocalReceiverType`, `Type::member` accepts `field` for c/cpp, and a singleton-only unique-field fallback covers untyped receivers. Full linux corpus: **+28,866 field nodes, +13,592 `field-call` edges**, failed dotted c/cpp `calls` **21,769 → 8,177**; a bare-name `field`→exact-match collision (~39k libc-name calls) was caught and gated; `unknown-receiver` genuine misses (local-var data-flow, a different problem); `Self::AssocType::*` binding, **done 2026-09-19** (§5.28).
6. **Product frontier** (non-resolver): dynamic-dispatch synthesis for reactive/reconciler runtimes (`ReactiveExtensionClient`, MediatR, Vue Proxy) per AGENTS.md. This is where effort moves the product metric. Every remaining resolver-migration item has been assessed as a permanent punt or an enhancement.

### Phase 5c: the remaining walker languages, DONE 2026-09-20

Phase 3's original order named Ruby, C# and Swift after C/C++, and the R7b ports added Scala, Dart, Lua/Luau and R walkers; none of the eight ever entered `is_migrated_language`, so on a C# corpus the kernel settled 25 refs and punted 3,126 (`ineligible:lang`, 0.8% native on eShop, measured before this phase). Like Rust before its `use` rows (Phase 5b R2), all eight are bindings-free on both engines: their import mappings are empty, every generic arm is a pure join, and the only language-specific TypeScript arms were small. Ported into `resolve.rs` and gated in:
...
6. **Supertype walks, DONE 2026-09-26** (ledger §5.67); **`via-src`, DONE 2026-09-26** (ledger §5.68: the object-literal alias and imported-instance arms run natively; vitest 93.0% → 96.9%); **`mc-await`, DONE 2026-09-26** (ledger §5.69: the sanitized scope walk runs natively; vitest 96.9% → 97.4%); **`mc-iteration`, DONE 2026-09-26** (ledger §5.70: the kernel walks its own tree; ktor 98.2% → 99.0%, exposed 96.4% → 97.2%); **`mc-guarded`, DONE 2026-09-26** (ledger §5.72; gated on laravel). Every `mc-*` punt is ported. **`php-inc`, DONE 2026-09-26** (ledger §5.73: the include-path arm; laravel 99.9% pooled, 100.0% sequential). matchBoundTypeMember's node-anchored BFS and resolveMethodOnType's name-union conformance recursion (depth < 4) run natively when the kernel's db holds every implements/extends edge the run reads: the live db, or a worker snapshot taken after the prerequisite phase (the only phase that writes them). The pool refreshes the workers' snapshot at the first idle boundary past that phase (recycle with a new path and generation; a fresh copy only if a prerequisite batch ran). Native share: ktor 96.5% → 98.2%, celery 99.0% → 99.6%, Ocelot 95.0% → 96.2%; every dump identical.
7. Retire the spine: `resolveOneInner` becomes kernel call → framework merge; the `name-matcher.ts` strategies and the parity suite's TS half go; the dump gate and golden dumps carry correctness. Scoped 2026-09-26 (census of every gate corpus plus a caller survey) into sub-legs, each landing on its own:
   - **7a. Incremental sync through the kernel, DONE 2026-09-26** (ledger §5.74). Sync resolved in TypeScript only; it now goes kernel-first, with a query-backed lookup mode that makes small syncs cheaper than the table load.
   - **7b. The conformance pass, DONE 2026-09-26** (ledger §5.75). The kernel's `defer` punt is now a deferring outcome settled with the framework merge, and the pass runs kernel-first (`resolveDeferredChains`). `defer` is gone from every corpus: Ocelot 96.2% → 99.8%, javalin 96.8% → 99.9%, exposed 97.3% → 99.2%.
   - **7c. Markdown, DONE 2026-09-26** (ledger §5.76). Link paths resolve through the ported file-path arm; admission was the change. vitest 97.3% → 99.7%, vite 97.9% → 99.9%, zod 98.8% → 99.9%; ktor, celery, Ocelot and javalin reach 100.0%.
   - **7d. Residual punts on migrated languages.** Part 1 **DONE 2026-09-26** (ledger §5.77): non-bare function refs, `this.<member>` included, and the `this.<member>` pass run natively; vitest, svelte, vite and zod reach 100.0%. The pool's pre-refresh supertype punts are gone too (ledger §5.78, laravel 109 → 0), and the store matchers run natively (ledger §5.79: exposed and svelte reach zero punts). The field-type tie-break now uses code-unit order in both engines (ledger §5.80: vitest and vite reach zero punts). Remaining after these: `member-tail` (store-accessor `().` receivers and others; vitest 287), `store-bind` and resolveStoreAction (exposed 282 JS), supertype walks in pool batches before the snapshot refresh (`rmot-`/`btm-supers`, laravel 109, vitest 101), `mc-tfield-ambig`, `claimed`, `gated-import`, `chain`.
   - **7e. The 18 unmigrated languages**. Group 1 **DONE 2026-09-26** (ledger §5.81: objc, solidity, liquid, twig, yaml, xml, properties admitted; every gate corpus 100% native). **Pascal DONE 2026-09-26** (ledger §5.82: built-in filter and receiver patterns ported; pasdoc 100% native). **Erlang DONE 2026-09-26** (ledger §5.83: arity prefilter, qualified-name, behaviour/.app and arity-call arms; cowboy 100% native). **COBOL, Nix, Terraform DONE 2026-09-26** (ledger §5.84: copybook and Nix path-import arms, the shared import-only rule, the Nix same-file name rule; carddemo, nixos-hardware, terraform-aws-vpc 100% native). **Razor DONE 2026-09-26** (ledger §5.85: the `@using` arm; eShopOnWeb 100% native). **CFML DONE 2026-09-26** (ledger §5.86: component paths, scope prefixes, receiver patterns via `cached_regex`; fw1 and TestBox 100% native). **VB.NET DONE 2026-09-26** (ledger §5.87; CompactGUI, staxrip 100% native). 7e is complete: every extractor language except the `unknown` placeholder is migrated. Originally: (razor, objc, pascal, solidity, vbnet, erlang, cfml/cfscript/cfquery, cobol, nix, liquid, twig, terraform, yaml, xml, properties) and their special arms (CFML component paths, COBOL copybooks, Nix path imports, Erlang arity).
   - **7f. Delete.** Parts 1–3a **DONE 2026-09-26** (ledger §5.88–§5.90): framework-claimed, gated-import and route refs settle through the framework merge, and the kernel settles every ref. Part 3b, step 1 **DONE 2026-09-26** (ledger §5.91): every resolution path (full index, pool workers, incremental sync, the public `resolveReferences`, both deferred passes) resolves through the kernel, a kernel that cannot open is an error, and `CODEGRAPH_KERNEL_RESOLVE=0` and `CODEGRAPH_RESOLVE_SHADOW=1` are gone. The gate's reference side is now the previous build. Step 2 **DONE 2026-09-26** (ledger §5.92): the unreachable spine is deleted — `resolveOne`/`resolveOneInner`, every `name-matcher.ts` strategy (the survivors, the language-family gates and binding helpers, moved to `gates.ts`), `resolveViaImport` and the other TypeScript import strategies, `receiver-iteration.ts`, and the context members only they read. expo-modules reaches the import arm through `context.resolveImport`, now backed by the kernel (`resolveViaImportRef`). `import-resolver.ts` keeps the shared import-path helpers the synthesizers and framework resolvers use. Leg 7 is complete.
