...
7. Retire the spine: `resolveOneInner` becomes kernel call → framework merge; the `name-matcher.ts` strategies and the parity suite's TS half go; the dump gate and golden dumps carry correctness. Scoped 2026-09-26 (census of every gate corpus plus a caller survey) into sub-legs, each landing on its own:
   ...
   - **7d. Residual punts on migrated languages.** Part 1 **DONE 2026-09-26** (ledger §5.77): non-bare function refs, `this.<member>` included, and the `this.<member>` pass run natively; vitest, svelte, vite and zod reach 100.0%. Remaining: `member-tail` (store-accessor `().` receivers and others; vitest 287), `store-bind` and resolveStoreAction (exposed 282 JS), supertype walks in pool batches before the snapshot refresh (`rmot-`/`btm-supers`, laravel 109, vitest 101), `mc-tfield-ambig`, `claimed`, `gated-import`, `chain`.
