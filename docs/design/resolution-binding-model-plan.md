...
7. Retire the spine: `resolveOneInner` becomes kernel call → framework merge; the `name-matcher.ts` strategies and the parity suite's TS half go; the dump gate and golden dumps carry correctness. Scoped 2026-09-26 (census of every gate corpus plus a caller survey) into sub-legs, each landing on its own:
   ...
   - **7e. The 18 unmigrated languages**. Group 1 **DONE 2026-09-26** (ledger §5.81: objc, solidity, liquid, twig, yaml, xml, properties admitted; every gate corpus 100% native). **Pascal DONE 2026-09-26** (ledger §5.82: built-in filter and receiver patterns ported; pasdoc 100% native). **Erlang DONE 2026-09-26** (ledger §5.83: arity prefilter, qualified-name, behaviour/.app and arity-call arms; cowboy 100% native). **COBOL, Nix, Terraform DONE 2026-09-26** (ledger §5.84: copybook and Nix path-import arms, the shared import-only rule, the Nix same-file name rule; carddemo, nixos-hardware, terraform-aws-vpc 100% native). Originally: (razor, objc, pascal, solidity, vbnet, erlang, cfml/cfscript/cfquery, cobol, nix, liquid, twig, terraform, yaml, xml, properties) and their special arms (CFML component paths, COBOL copybooks, Nix path imports, Erlang arity).
