...
- **Outcome semantics under frameworks**: `unresolved` is terminal only for the pre-framework early-outs (builtin, prefilter-fail when frameworks are inactive). An exhausted pipeline under active frameworks reports `candidates: []` (the `no_candidates` marker) so the TS merge still runs the framework loop; a gated-out ≥0.9 import under frameworks reports `passthrough` (a ≥0.9 framework hit would have pre-empted it in the spine); a gated-out merged winner reports its raw candidate list so TS can re-run first-max with framework candidates prepended. `isFinal` marks the ≥0.9 import early-win — a <0.9 framework candidate cannot displace it, a ≥0.9 one can.
...
**Measured (2026-09-15, linux-kernel corpus @58785836, 15c/47GB host):** 10 corpora byte-identical (edges + unresolved_refs incl. row-id order), `CODEGRAPH_RESOLVE_SHADOW=1` reports 0 divergences over ~197k kernel-handled refs; eval:precision holds every case on all 10. Resolution phase 281s kernel-on vs 229s kernel-off — the single-threaded `resolveChunk` loses the pool's cross-worker parallelism and its settle no longer overlaps the previous batch's persist. Kernel settle itself is ~10µs/ref (20.5s for ~2M refs). Follow-up to reclaim the parallelism: give each resolver-pool worker its own `KernelResolver` (napi classes work in worker_threads) so the eligible slice resolves natively across cores instead of serially on the main thread.
