...
3. **Rust bindings emission** (`rustlang.rs` → `bindings` rows) — **LANDED as enhancement 2026-09-19** (§5.29). `use` declarations emit one `import` row per bound local name on both walks (item + fn-body); `rust` joined `BINDINGS_LANGUAGES` + `bindingsFile` dispatch. Corpus dual-db A/B: +69 resolved / −0 across 3.83M edges, 0 shadow divergences on 11,942 handled; the new edge class (bound name, no in-graph target → fuzzy @0.5) is standard weakest-tier behavior.
4. **Adjacent enhancement candidates** (different axis — node-set changes): C `ops->read` fn-ptr field calls (~12k refs, §5.22 — needs field nodes + C receiver inference); `unknown-receiver` genuine misses (local-var data-flow — a different problem entirely); `Self::AssocType::*` binding — **done 2026-09-19** (§5.28).
