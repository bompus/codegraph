...
7. Retire the spine: `resolveOneInner` becomes kernel call → framework merge; the `name-matcher.ts` strategies and the parity suite's TS half go; the dump gate and golden dumps carry correctness. Scoped 2026-09-26 (census of every gate corpus plus a caller survey) into sub-legs, each landing on its own:
   ...
   - **7c. Markdown.** The largest punt left on every JS/TS corpus (`ineligible:lang`): vitest 2,060, vite 890, zod 431, exposed 381, svelte 370.
   ...
   - **7e. The 18 unmigrated languages** (razor, objc, pascal, solidity, vbnet, erlang, cfml/cfscript/cfquery, cobol, nix, liquid, twig, terraform, yaml, xml, properties, markdown after 7c) and their special arms (CFML component paths, COBOL copybooks, Nix path imports, Erlang arity).
