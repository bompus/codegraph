...
This table records the historical trace-era evaluation; current validation uses
`codegraph_explore` and its Flow section as described above.
...
n=4 unhooked runs/stage, same prompt. Run-to-run variance is real; report the range, never a single run. **Residual reads/greps are all the nonce data-flow** (`canvasNonce` — a local prop with no graph edges); that's the def-use/data-flow frontier, left deliberately uncovered (tracking every local would explode the graph). The same six-hop flow is now validated through `codegraph_explore`: `mutateElement → triggerUpdate → [callback] triggerRender → [react-render] render → [jsx] StaticCanvas → renderStaticScene`; node count stable at 9,289; 1 callback + 46 react-render + 280 jsx-render synthesized edges (no explosion, precision-checked).
