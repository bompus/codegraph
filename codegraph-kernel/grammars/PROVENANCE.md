...
| cobol | yutaro-sakamoto/tree-sitter-cobol | e99dbdc3 + docs/grammars/tree-sitter-cobol.patch | regenerated with tree-sitter-cli 0.25.10 (ABI 14); see docs/grammars/tree-sitter-cobol.md. `scanner.c` additionally carries a two-line portability patch (fixed-size arrays instead of C99 VLAs in `start_with_word`), because MSVC rejects VLAs; the wasm build used clang, which accepts them |
