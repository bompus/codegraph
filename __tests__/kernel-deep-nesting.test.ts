...
describe.skipIf(!kernelBuilt)("kernel deep-nesting guard (#1581)", () => {
  let routed: Language[] = [];
  const savedKernel = process.env.CODEGRAPH_KERNEL;
  ...
  it("every default-routed language survives a 60k-deep expression on the main thread", () => {
    ...
    for (const language of routed) {
      ...
      const fn = result.nodes.find(
        (n) => n.name === "f" && (n.kind === "function" || n.kind === "method"),
      );
      ...
      if (!fn && language !== "r")
        failures.push(
          `${language}: no function node 'f' (nodes=${result.nodes.map((n) => `${n.kind}:${n.name}`).join(",")})`,
        );
  ...
  describe.skipIf(!distBuilt)("inside a default-sized (4 MiB) parse worker, through dist/", () => {
    ...
    function runInWorker(
      ...
      source: string,
      language: Language,
    ): Promise<{ exitCode: number; outcome: string }> {
  ...
  describe.skipIf(!distBuilt)("end-to-end: codegraph init on a repo holding the deep file", () => {
    ...
    it("exits 0 and records deep.c alongside the normal files", () => {
      ...
      try {
        const files = (
          db.prepare("SELECT path FROM files ORDER BY path").all() as Array<{ path: string }>
        ).map((r) => r.path);
