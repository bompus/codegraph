 ...
 */
describe.skipIf(!kernelBuilt)('AST-only binding rows agree with the walker (JS family, Python, Go)', () => {
  const key = (b: Binding) =>
    [b.kind, b.name, b.scopeStart, b.scopeEnd, b.line, b.targetSpec ?? '', b.targetName ?? '', b.exportedAs ?? '', b.exportForm ?? '', b.nodeId ?? ''].join('|');
  const JS_EXT: Record<string, Language> = { '.js': 'javascript', '.jsx': 'jsx', '.ts': 'typescript', '.tsx': 'tsx', '.py': 'python', '.go': 'go' };
  const fixtures = fs
    .readdirSync(FIXTURE_DIR)
