...
- An explicitly named receiver type can disambiguate its capitalized method from lowercase prose; named definitions include bounded same-file caller/callee context.
...
- **"Why is this like this? What did the last session decide / try / get told about X?"** → \`codegraph_sessions\` with a few words. It searches the prose of this project's earlier Claude Code, Codex, Cursor/T3, OpenCode, and AGY sessions (prompts, replies, compaction summaries — stemmed, ranked) and names the session each hit came from (\`claude:\`, \`codex:\`, \`cursor:\`, \`opencode:\`, \`agy:\`). History and rationale live there, not in the code; do not grep transcript files by hand.
