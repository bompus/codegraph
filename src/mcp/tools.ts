...
export interface ExploreAllocationCandidate {
   ...
   * construction). Exempt from the cliff like a spine file — the cliff is a
   * relative prune of weak GRAPH evidence, which injection deliberately lacks —
...
export const tools: ToolDefinition[] = [
  ...
  },
  {
    ...
    description: 'Returns the verbatim, line-numbered source of the code relevant to a query, grouped by file (the same `<n>\\t<line>` shape as Read, safe to Edit from), plus the call path among the named symbols — including dynamic-dispatch hops grep cannot follow — and, when it fits, a summary of what depends on them. Use it first for questions about indexed code (how X works, where X is, a flow, a bug, a documentation section) and to load the symbols you are about to change. The query is a natural-language question or a bag of symbol names and file paths; named items are prioritised, and a question about the working changes ("my changes", "this branch", `main..HEAD`) is answered from the git diff. Output is capped by project size (about 13K–24K characters); files that did not fit may be listed by name for a follow-up call. Treat returned ranges as already Read, except files the response flags as pending sync or changed on disk. It does not return files codegraph does not index, such as most configs. One call usually covers a question.',
    inputSchema: {
      type: 'object',
      properties: {
        query: {
        ...
        maxFiles: {
          type: 'number',
          description: 'Maximum number of files to return source from, 1–20. Omit it to use a default sized to the project (4 files below 150 indexed files, 5 below 500, 8 otherwise); when omitted and the query quotes a literal, the cap rises to the number of files holding it, up to 12. Total output is also capped by characters (about 13K–24K by project size), so a higher maxFiles spreads that budget over more files rather than returning more text.',
        },
        projectPath: projectPathProperty,
      },
      required: ['query'],
...
export class ToolHandler {
  ...
  private async handleExplore(args: Record<string, unknown>): Promise<ToolResult> {
    ...
    // Bare-token path-segment seeds (weakest tier) — see the seeding block.
    const pathVocabSeedIds = new Set<string>();
    // Nodes pulled by the importer-spine walk off the files a bare directory
    // token matched — the files that wire a query-named directory/module (the
    // registry that `import()`s it, the entrypoint importing that registry) —
    // keyed to the hop depth they were reached at (1 = imports a matched file
    // directly).
    // Mid tier: stronger evidence than a generic connected node, weaker than
    // the file the agent actually named, and weaker with every hop.
    const spineNodeDepth = new Map<string, number>();
    ...
    const namedTypeFiles = new Set<string>();
    {
      ...
      // files may share NO token with the query (a `COMMANDS` registry
      // object's members carry none of the words "command registry lookup"),
      // so the flow stays invisible until the agent spells the path. Seed
      // each matched file's most-central seedable node into the restart set —
      // the RWR walk then accrues mass on it and its dispatch-linked
      // neighbors (the registry's own edges pull the hub into view). Bare
      // tokens and directory segments only — a precise token already seeds
      // by name, and a filename-stem match is too easy to collide with
      // (`string` → `transforms/string.ts` is noise, not location). Weakest
      // tier: restart seeds and gate protection, never named-first.
      {
        const bareTokens = tokens.filter((t) => t.length >= 4 && !isPreciseToken(t) && !STOP_WORDS.has(t.toLowerCase()));
        if (bareTokens.length > 0) {
          // One pass over the file table: directory segment → files under it
          // (test and generated files left out — generated bytes are worth
          // ~nothing, which is what the worth penalty and cliff exist to
          // express, and a bare directory token is the weakest evidence tier,
          // so it never injects one). Per token the lookup is then O(1)
          ...
          for (const f of cg.getFiles()) {
            ...
            segs.pop();
            for (const dir of new Set(segs)) {
              const bucket = filesByDirSegment.get(dir);
              if (bucket) bucket.push(f.path); else filesByDirSegment.set(dir, [f.path]);
            }
          }
          ...
          for (const t of bareTokens) {
            ...
            // Exact segment or a simple plural (`command` → `commands/`).
            // Prefix-matching whole segments (`payroll` → `payrolld/`) pulls
            // in files the token does not name — observed displacing the
            // CG-12 slot hand-off in the #1500 payroll fixture.
            const forms = new Set([lt, `${lt}s`, `${lt}es`, lt.endsWith('s') ? lt.slice(0, -1) : lt]);
            const inDir = [...new Set([...forms].flatMap((d) => filesByDirSegment.get(d) ?? []))]
              .filter((fp) => !matched.has(fp))
              .sort();
            if (inDir.length === 0) continue;
            // Files the query's own channels already surfaced in the named
            // directory stand on their organic evidence, and they are the
            // natural spine STARTS (their importer chain is the invisible
            // part). Seeding siblings beside them adds nothing to the spine —
            // same importers — and re-seeding them perturbs the restart mass
            // of files already ranked on merit (#1500's `usecase/payroll/
            // cycle.go` under `payroll` shifted from clusters to whole-file
            // and reshaped every sibling's allocation). Only a directory with
            // NO organic hit gets seeded, two files at most.
            const surfaced = inDir.filter((fp) => subgraphFiles.has(fp));
            if (surfaced.length > 0) {
              for (const fp of surfaced.slice(0, 6 - matched.size)) matched.add(fp);
              continue;
            }
            let picked = 0;
            for (const fp of inDir) {
              if (picked >= 2 || matched.size >= 6) break;
              let fileNodes: Node[] = [];
              try { fileNodes = cg.getNodesInFile(fp); } catch { continue; }
              const seed = fileNodes
                .filter((n) => SEEDABLE.has(n.kind) && !isInterfaceOwnedMethod(n))
                .sort((a, b) => callerCount(b) - callerCount(a) || bodyLines(b) - bodyLines(a))[0];
              if (!seed) continue;
              matched.add(fp);
              picked++;
              subgraphFiles.add(fp);
              if (!subgraph.nodes.has(seed.id)) subgraph.nodes.set(seed.id, seed);
              pathVocabSeedIds.add(seed.id);
              // File-level `imports` edges (incl. dynamic `import('./x')`)
              // land on the file node, not the symbol — entering it too lets
              // the importer spine below score as entry-connected.
              const fileSeed = fileNodes.find((n) => n.kind === 'file');
              if (fileSeed) {
                if (!subgraph.nodes.has(fileSeed.id)) subgraph.nodes.set(fileSeed.id, fileSeed);
                pathVocabSeedIds.add(fileSeed.id);
              }
              glueRootIds.add(seed.id);
              // A bare node injection is an island — the RWR walk only crosses
              // edges already in the subgraph, so the dispatch hub the seed
              // links to (the registry's `main`/`COMMANDS`) would stay
              // unreachable. Pull the seed's usage-edge endpoints in WITH it.
              try {
                let pulled = 0;
                for (const e of [...cg.getIncomingEdges(seed.id), ...cg.getOutgoingEdges(seed.id)]) {
                  if (pulled >= 8 || e.source === e.target || !RELEVANCE_USAGE_EDGES.has(e.kind)) continue;
                  const otherId = e.source === seed.id ? e.target : e.source;
                  const other = cg.getNode(otherId);
                  if (!other) continue;
                  if (!subgraph.nodes.has(otherId)) {
                    subgraph.nodes.set(otherId, other);
                    subgraphFiles.add(other.filePath);
                    pulled++;
                  }
                  subgraph.edges.push(e);
                }
              } catch { /* neighborhood pull is best-effort */ }
            }
          }
          // Importer spine: the hub a path-seeded file belongs to reaches it
          // through a file-level `imports` edge (static `import …` or dynamic
          // `import('./x')`), which lands on the FILE node — not the symbol seed
          // pulled above. Walk inbound `imports` on each surfaced file's file
          // node, and on each importer's own file (the importer's importer is
          // usually the entrypoint: about.ts → COMMANDS in cmd.config.ts →
          // main.ts). Strictly bounded by the seed count and SPINE_BUDGET.
          const SPINE_BUDGET = 8;
          let spineBudget = SPINE_BUDGET;
          const spineQueue: Array<[string, number]> = [...matched].map((fp) => [fp, 0]);
          const spineSeen = new Set<string>();
          while (spineQueue.length > 0 && spineBudget > 0) {
            const [fp, depth] = spineQueue.shift()!;
            if (spineSeen.has(fp)) continue;
            spineSeen.add(fp);
            let fileNode: Node | undefined;
            try { fileNode = cg.getNodesInFile(fp).find((n) => n.kind === 'file'); } catch { continue; }
            if (!fileNode) continue;
            let inbound: Edge[] = [];
            try { inbound = cg.getIncomingEdges(fileNode.id); } catch { continue; }
            for (const e of inbound) {
              if (spineBudget <= 0) break;
              if (e.kind !== 'imports' || e.source === e.target) continue;
              const importer = cg.getNode(e.source);
              if (!importer || importer.kind === 'import') continue;
              spineBudget--;
              // A generated importer stays walked-through (its own importer
              // may be the entrypoint) but never spine-marked: the mark is
              // cliff immunity, which generated boilerplate must not earn.
              const importerGenerated = generatedPaths.has(importer.filePath);
              if (!importerGenerated) {
                if (!subgraph.nodes.has(importer.id)) subgraph.nodes.set(importer.id, importer);
                subgraph.edges.push(e);
                spineNodeDepth.set(importer.id, depth + 1);
                // When the importer edge is file→file the useful member stays
                // invisible — the file node alone can't carry the group. Pull
                // the importer file's hub symbol so `main.ts` contributes `main`
                // rather than an anonymous file node.
                if (importer.kind === 'file' && spineBudget > 0) {
                  let hub: Node[] = [];
                  try { hub = cg.getNodesInFile(importer.filePath); } catch { hub = []; }
                  const top = hub
                    .filter((n) => SEEDABLE.has(n.kind) && !isInterfaceOwnedMethod(n))
                    .sort((a, b) => callerCount(b) - callerCount(a) || bodyLines(b) - bodyLines(a))[0];
                  if (top && !subgraph.nodes.has(top.id)) {
                    spineBudget--;
                    subgraph.nodes.set(top.id, top);
                    spineNodeDepth.set(top.id, depth + 1);
                  }
                }
              }
              // A file already surfaced by another channel (e.g. the registry
              // was itself an FTS hit) still needs WALKING — its inbound
              // importer chain is what reaches the entrypoint. subgraphFiles
              // membership says "surfaced", spineSeen says "walked" — only the
              // second dedupes the queue.
              if (!spineSeen.has(importer.filePath)) {
                subgraphFiles.add(importer.filePath);
                spineQueue.push([importer.filePath, depth + 1]);
              }
            }
          }
        }
    ...
    const fileGroups = new Map<string, { nodes: Node[]; score: number; peripheral: number; injected: number }>();
    // Organic entries — what the query's own channels matched. Path-vocab seeds
    // join the wider `entryNodeIds` (restart set, connectivity, gate) but score
    // on the injected ledger below, so they only reach the entry tier here when
    // a query channel found them too.
    const organicEntryIds = new Set([...subgraph.roots, ...namedSeedIds]);
    const entryNodeIds = new Set([...organicEntryIds, ...pathVocabSeedIds]);
    ...
    const isUsageIsolated = (node: Node): boolean => {
      ...
      let isolated = true;
      try {
        // An OUTGOING `imports` edge counts as usage here even though the kind
        // stays out of RELEVANCE_USAGE_EDGES (which also gates neighborhood
        // fan-out): a registry constant whose body is `load: () => import('./x')`
        // specifiers is wired to every module it loads — the opposite of
        // isolated. Incoming imports and `exports` edges do not count: being
        // importable says nothing about being used.
        const used = (e: Edge) => RELEVANCE_USAGE_EDGES.has(e.kind);
        isolated = !cg.getIncomingEdges(node.id).some(used)
          && !cg.getOutgoingEdges(node.id).some((e) => used(e) || e.kind === 'imports');
      } catch {
    ...
    for (const node of subgraph.nodes.values()) {
      ...
      const group = fileGroups.get(node.filePath) || { nodes: [], score: 0, peripheral: 0, injected: 0 };
      ...
      //
      // Path-vocab seeds and importer-spine pulls score on a SEPARATE, flat
      // ledger (`group.injected`) that becomes a FLOOR on the file's score —
      // never a bonus on top of organic evidence. Flat, because a path-segment
      // match is location evidence about the FILE and the winning symbol's kind
      // is incidental (a commands/ registry held in a `constant` is still the
      // registry the agent named; kind-weighting it at 0.35 lands under the
      // floor). A floor, because a file that ALSO matched organically (#1500's
      // `usecase/payroll/cycle.go` under a `payroll` token) must keep its
      // organic score: adding +10 on top inflated the top score, dragged the
      // relative floor and cliff up with it, and starved the honest mid-rank
      // `payslip_builder.go` the fixture guards.
      //
      // Within the ledger, wiring outranks location: a spine hop is
      // graph-attested (this file imports the matched one), a path seed is
      // "some file in a directory the token names" — the walk's starting
      // point, not an answer. Each further hop is worth less; the file node
      // of a path seed scores nothing (it is there for the spine's edges).
      const hop = spineNodeDepth.get(node.id);
      if (hop !== undefined) group.injected += Math.max(3, 9 - 2 * hop);
      else if (pathVocabSeedIds.has(node.id) && node.kind !== 'file') group.injected += 5;
      if (namedSeedIds.has(node.id)) {
        group.score += 50 * relevanceWeight(node, true);
      } else if (organicEntryIds.has(node.id)) {
    ...
    // Files the path-vocab/importer-spine seeding CARRIED: their injected floor
    // beat whatever the query's own channels scored. These are exempt from the
    // score floor, the graph-mass gate and the allocation cliff (pulled BECAUSE
    // the walk can't reach them lexically, so zero RWR mass is expected) and
    // rank in a bounded sort tier of their own. A file that also matched
    // organically is not injected — it stands on its own evidence.
    const injectedFiles = new Set<string>();
    for (const [filePath, group] of fileGroups) {
      const organic = group.score + Math.min(PERIPHERAL_SCORE_CAP, group.peripheral);
      if (group.injected > organic) injectedFiles.add(filePath);
      group.score = Math.max(organic, group.injected) * rankPenalty(filePath);
    }
    ...
    for (const t of changeSurfaceCandidates) {
      ...
      if (!group) { group = { nodes: [], score: 0, peripheral: 0, injected: 0 }; fileGroups.set(fp, group); }
    ...
    // Injected (path-vocab/importer-spine) files carry zero RWR mass BY
    // CONSTRUCTION — the walk couldn't reach them — so sorting them on graph
    // score sinks them below the cut they were injected to survive. They get a
    // tier of their own, but a BOUNDED one: injection is survival, not
    // promotion. At most half the file slots go to it, best wiring evidence
    // first (the registry and entrypoint, not the third hop or an alphabetical
    // sibling in the named directory); the rest fall through to the graph
    // tiers like any other candidate.
    const injectedTier = new Set(
      [...relevantFiles]
        .filter(([fp]) => injectedFiles.has(fp))
        .sort((a, b) => b[1].injected - a[1].injected
          || (fileTermHits.get(b[0]) ?? 0) - (fileTermHits.get(a[0]) ?? 0)
          || b[1].nodes.length - a[1].nodes.length)
    ...
    const sortedFiles = relevantFiles.sort((a, b) => {
      ...
      // Bounded injected tier (see `injectedTier`): below files the query
      // lexically corroborated, above the graph signal that can't see them.
      // Inside it graph mass is meaningless — order on the wiring evidence.
      const aInj = injectedTier.has(a[0]) ? 1 : 0;
      const bInj = injectedTier.has(b[0]) ? 1 : 0;
      if (aInj !== bInj) return bInj - aInj;
      if (aInj && bInj && a[1].injected !== b[1].injected) return b[1].injected - a[1].injected;
