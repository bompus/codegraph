...
/** The extraction, resolution and watcher modules, loaded on first use. */
interface Stacks {
  extraction: typeof import('./extraction');
  resolution: typeof import('./resolution');
  sync: typeof import('./sync');
}
let stacks: Stacks | undefined;

/**
 * The public entry (`./index`) hands over the stacks it already imports. A
 * read-only caller that loads this module directly — a query worker, the MCP
 * engine — never parses, resolves or watches until it has to, and skipping
 * those modules saves each worker thread tens of MB.
 */
export function provideStacks(s: Stacks): void {
  stacks = s;
}

const loadStacks = (): Stacks =>
  (stacks ??= { extraction: require('./extraction'), resolution: require('./resolution'), sync: require('./sync') });
const extraction = () => loadStacks().extraction;
const resolution = () => loadStacks().resolution;
const sync = () => loadStacks().sync;
...
export class CodeGraph {
  ...
  private lazyOrchestrator?: ExtractionOrchestrator;
  private lazyResolver?: ReferenceResolver;
  private get orchestrator(): ExtractionOrchestrator {
    return (this.lazyOrchestrator ??= new (extraction().ExtractionOrchestrator)(this.projectRoot, this.queries));
  }
  ...
  private wireLayers(): void {
    ...
    // Built on first use against the current `queries` (see the getters).
    this.lazyOrchestrator = undefined;
    this.lazyResolver = undefined;
      ...
      try {
        ...
        if (result.success && result.filesIndexed > 0) {
          try {
            ...
            const head = extraction().getGitHeadSha(this.projectRoot);
            if (head) this.queries.setMetadata(extraction().INDEXED_AT_COMMIT_KEY, head);
